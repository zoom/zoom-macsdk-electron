...
var ZoomAuthBJ = (function () {
    ...
    SetAuthCB: function(onAuthResult)
    {
        ...
        zoomSDKAuthDelegate.addMethod('onZoomSDKAuthReturn:', 'v@:i', function (self, _cmd, returnValue) {
            if(returnValue == 0)
    ...
    SetLoginCB: function(onLoginResult)
    {
        var zoomLoginReturn = ZOOMAUTHMOD.ZoomLoginStatus.LOGIN_IDLE;
            var zoomSDKAuthDelegate = ZOOMSDK.authServiceDelegate;
            zoomSDKAuthDelegate.addMethod('onZoomSDKLogin:failReason:', 'v@:i@', function (self, _cmd, loginStatus, failReason) {
                if(loginStatus == 0)
